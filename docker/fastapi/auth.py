    ...
    if conn is not None:
        try:
            with conn.cursor() as cur:
                ...
                cur.execute(
                    "INSERT INTO users (username, email, hached_password) VALUES (%s, %s, %s)",
                    (
    ...
    if conn is not None:
        try:
            with conn.cursor() as cur:
                ...
                logger.info(
                    f"Type de userId dans authenticate_user: {type(user_data['userId'])}"
                )
                logger.info(
                    f"Valeur de userId dans authenticate_user: {user_data['userId']}"
                )
                return user_data
...
async def get_current_user(token: Annotated[str, Depends(oauth2_bearer)]):
    try:
        ...
        if conn is not None:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT userid, username FROM users WHERE userid = %s", (user_id,)
                )
                user = cur.fetchone()
                username = user[1]
        if email is None or user_id is None:
            ...
            )  # Erreur si les donnees sont manquantes
        response = {
            "email": email,
            "id": user_id,
            "username": username,
